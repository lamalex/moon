...
async fn load_bundled_vcs_plugin(
    ...
    registry.load_without_config(Id::raw(id), locator).await
}
...
mod tests {
    ...
    fn require_jj() {
        let output = Command::new("jj")
            .arg("--version")
            .output()
            .expect("Jujutsu 0.44.0 must be installed to run VCS provider tests");
        assert!(
            output.status.success(),
            "Jujutsu failed to report its version"
        );
        assert!(
            String::from_utf8_lossy(&output.stdout).starts_with("jj 0.44."),
            "VCS provider tests require Jujutsu 0.44.x, got {}",
