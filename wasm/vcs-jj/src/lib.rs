...
pub fn initialize_vcs(
    ...
    let initialization = VcsInitialization {
        ...
        roots: VcsRoots {
            repository_root: VirtualPath::new(normalize_native_path(root.stdout.trim())),
            working_root: VirtualPath::new(normalize_native_path(root.stdout.trim())),
        },
        current,
        recorded,
        baseline,
...
fn history_completeness(context: &MoonContext, observation_id: &str) -> VcsHistoryCompleteness {
    ...
    let Ok(output) = run_git_in_repository(
        context,
        &normalize_native_path(git_root.stdout.trim()),
...
fn get_workspace_prefix(context: &MoonContext, observation_id: &str) -> AnyResult<String> {
    ...
    require_success(&root)?;
    let workspace_root = context.workspace_root.as_path().to_string_lossy();
    let repository_root = normalize_native_path(root.stdout.trim());
    let relative = native_relative_path(&workspace_root, &repository_root).ok_or_else(|| {
        anyhow!(
...
fn git_repository_slug(
        ...
        .chain(["origin", "upstream"])
    {
        ...
        let output =
            run_git_in_repository(context, &git_root, ["remote", "get-url", remote]).ok()?;
...
fn jj_git_root(context: &MoonContext, observation_id: &str) -> Option<String> {
    ...
    Some(normalize_native_path(output.stdout.trim()))
}

fn normalize_native_path(path: &str) -> String {
    let path = path.replace('\\', "/");
    let (prefix, remainder) = if matches!(path.as_bytes(), [drive, b':', b'/', ..] if drive.is_ascii_alphabetic())
    {
        (&path[..3], &path[3..])
...
fn native_relative_path(path: &str, base: &str) -> Option<String> {
    let path = normalize_native_path(path);
    let base = normalize_native_path(base);

    for (path, base) in [
        (path.as_str(), base.as_str()),
        (
            path.strip_prefix("/private").unwrap_or(&path),
            base.strip_prefix("/private").unwrap_or(&base),
        ),
    ] {
        let windows = matches!(path.as_bytes(), [drive, b':', b'/', ..] if drive.is_ascii_alphabetic())
            || path.starts_with("//");
        let matches = path.get(..base.len()).is_some_and(|prefix| {
            if windows {
        ...
        if matches
            && let Some(relative) = path.get(base.len()..).and_then(|suffix| {
                suffix
                    .strip_prefix('/')
...
where
    ...
    S: Into<String>,
{
    run_git(context, git_repository_args(git_root, args))
}

fn git_repository_args<I, S>(git_root: &str, args: I) -> Vec<String>
where
    I: IntoIterator<Item = S>,
    S: Into<String>,
{
    std::iter::once(format!("--git-dir={git_root}"))
...
mod tests {
    ...
    #[test]
    fn constructs_git_commands_for_an_external_backend_from_the_workspace() {
        let cwd = VirtualPath::new("C:/repo/workspace");
        let input = git_command(
            cwd.clone(),
            git_repository_args(
                "C:/repo/.jj/repo/store/git",
                ["rev-parse", "--is-shallow-repository"],
            ),
        );

        assert_eq!(input.cwd, Some(cwd));
        assert_eq!(
            input.args,
            [
                "-c",
                ...
                "-c",
                "status.relativePaths=false",
                "--git-dir=C:/repo/.jj/repo/store/git",
                "rev-parse",
    ...
    #[test]
    fn handles_native_roots_independently_of_guest_path_semantics() {
        assert_eq!(
            normalize_native_path(r"C:\repo\workspace"),
            "C:/repo/workspace"
        );
        assert_eq!(
            native_relative_path("C:/repo/workspace/app", r"c:\repo\workspace"),
            Some("app".into())
        );
        assert_eq!(
            native_relative_path(r"\\server\share\repo\workspace", "//server/share/repo"),
            Some("workspace".into())
        );
        assert_eq!(native_relative_path("C:/repo-other", "C:/repo"), None);
        assert_eq!(
            native_relative_path("/private/var/repo/workspace", "/var/repo"),
            Some("workspace".into())
        );
    }
