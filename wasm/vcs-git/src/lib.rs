...
pub fn initialize_vcs(
    ...
    let working_root = normalize_native_path(&repository_root);
    let repository_root = git_repository_root(&input.context)?;
    let initialization = VcsInitialization {
        client: Id::raw("git"),
        client_version: git_version(&input.context),
        roots: VcsRoots {
            repository_root,
            working_root: VirtualPath::new(&working_root),
        },
...
pub fn setup_vcs_hook_environment(
    ...
    Ok(Json(SetupVcsHookEnvironmentOutput {
        working_dir: Some(VirtualPath::new(initialization.working_root)),
    }))
}
...
fn git_repository_root(context: &MoonContext) -> AnyResult<VirtualPath> {
    let output = run_git(context, git_args(["rev-parse", "--git-common-dir"]), false)?;
    require_success(&output)?;
    let common_dir = normalize_native_path(stdout_text(&output)?.trim());
    let common_dir = if is_native_absolute_path(&common_dir) {
        VirtualPath::new(common_dir)
    } else {
        VirtualPath::new(normalize_native_path(
            &context
                .workspace_root
                .join(common_dir)
                .as_path()
    ...
    let repository_root = if common_dir
        .as_path()
        .file_name()
...
fn working_changes(
    ...
    for submodule in submodule_paths(context)? {
        ...
        let cwd = context.workspace_root.join(relative);
        let output = run_git_at(cwd.clone(), status_args(), true)?;
        ...
        let Some(gitlink_change) = changes.get(&submodule).copied() else {
            continue;
        };
        let Some(recorded) = submodule_commit(context, "HEAD", &submodule)? else {
            continue;
        };
        let Some(checked_out) = successful_stdout(run_git_at(
            cwd.clone(),
            git_args(["rev-parse", "--verify", "HEAD^{commit}"]),
            false,
        )?) else {
        ...
        let output = run_git_at(cwd, diff_args(&recorded, &checked_out), false)?;
        require_success(&output)?;
        let layers = gitlink_change & (VcsChangeMask::STAGED | VcsChangeMask::WORKING);
        let mut gitlink_changes = parse_diff(stdout_text(&output)?);

        for mask in gitlink_changes.values_mut() {
            mask.remove(VcsChangeMask::RECORDED);
            *mask |= layers;
        }

        changes.remove(&submodule);
        merge_changes(&mut changes, prefix_changes(gitlink_changes, &submodule));
    }
...
mod tests {
    ...
    #[test]
    fn recognizes_and_normalizes_native_absolute_paths() {
        assert!(is_native_absolute_path("/repo/workspace"));
        assert!(is_native_absolute_path("C:/repo/workspace"));
        assert!(is_native_absolute_path("z:/repo/workspace"));
        assert!(is_native_absolute_path("//server/share/repo"));
        assert!(!is_native_absolute_path("../repo/workspace"));
        assert!(!is_native_absolute_path("C:repo/workspace"));
        assert_eq!(
        ...
        assert_eq!(
            normalize_native_path("C:/repo/workspace/../.git"),
            "C:/repo/.git"
        );
        assert_eq!(
            normalize_native_path("//server/share/repo/../metadata"),
            "//server/share/metadata"
        );
        assert_eq!(normalize_native_path("../.git"), "../.git");
    }

    #[test]
    fn preserves_submodule_commit_changes_in_status() {
        assert!(
            status_args()
                .iter()
                .any(|arg| arg == "--ignore-submodules=dirty")
        );
    }
